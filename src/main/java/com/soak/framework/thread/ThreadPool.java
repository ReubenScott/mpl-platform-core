 ...
 * 
 * 工作原理: 线程池的工作中主要涉及到:corePoolSize,workQueue,maximumPoolSize,
 * RejectedExecutionHandler 它们的调用原理: 1 当线程池中线程数量小于corePoolSize则创建线程,并处理请求 2
 * 当线程池中线程数量等于corePoolSize则把请求放入workQueue中,线程池中的的空闲线程就从workQueue中取任务并处理 3
 * 当workQueue已满存放不下新入的任务时则新建线程入池,并处理请求;
 * 如果线程池中线程数大于maximumPoolSize则用RejectedExecutionHandler使用一定的策略来做拒绝处理
 * 
 * 在该机制中还有一个keepAliveTime,文档描述如下: when the number of threads is greater than the
 * core, this is the maximum time that excess idle threads will wait for new
 * tasks before terminating. 它是什么意思呢？ 比如线程池中一共有5个线程,其中3个为核心线程(core)其余两个为非核心线程.
 ...
 * 
 * 参考资料: http://blog.csdn.net/cutesource/article/details/6061229
