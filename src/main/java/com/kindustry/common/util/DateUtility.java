...
import com.kindustry.system.constant.Week;

public class DateUtility {

  private final static Logger logger = LoggerFactory.getLogger(DateUtility.class);
  ...
  public static Date getClosestPreviousTime(Date referenceTime, List<Date> dates) {
    Date prior = null;
    for (Date date : dates) {
      float diff = DateUtility.timeDiff(referenceTime, date);
      if (diff <= 0) {
  ...
  public static int getIntervalDays(Date date, Date otherDate) {
    date = DateUtility.parseDateString(DateUtility.getShortDate(date), DateStyle.SHORTDATEFORMAT);
