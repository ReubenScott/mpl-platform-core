...
public class StringUtility {
  ...
  /**
   * 到此可以发先StringBuilder的强大作用 删除inString中在charsToDelete中存在的字符 例如 inString = "abddfkjfd"; charsToDelete = "cdjf"; 则处理后的inString = "abk"
   */
  ...
  /**
   * 获取给定的字符串中，最后一个满足分隔符separator之后字符串， 例如 qualifiedName = "this:name:is:qualified" separator = ':' 那么处理过后的字符串就是 qualified
   */
   ...
   * <p>
   * The given delimiters string is supposed to consist of any number of delimiter characters. Each of those characters can be used to separate tokens. A delimiter is always a
   * single character; for multi-character delimiters, consider using <code>delimitedListToStringArray</code>
   * 
   * @param str
   ...
   *          omit empty tokens from the result array (only applies to tokens that are empty after trimming; StringTokenizer will not consider subsequent delimiters as token in the
   *          first place).
