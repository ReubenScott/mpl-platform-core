 ...
 * 
 * 在该机制中还有一个keepAliveTime,文档描述如下: when the number of threads is greater than the core, this is the maximum time that excess idle threads will wait for new tasks before terminating.
 * 它是什么意思呢？ 比如线程池中一共有5个线程,其中3个为核心线程(core)其余两个为非核心线程. 当超过一定时间(keepAliveTime)非核心线程仍然闲置(即没有执行任务或者说没有任务可执行)那么该非核心线程就会被终止. 即线程池中的非核心且空闲线程所能持续的最长时间,超过该时间后该线程被终止.
 * 
 * 
...
public class ThreadPool {
  ...
  private static final int POOLSIZE = Runtime.getRuntime().availableProcessors(); // 可用cpu逻辑处理器 , 线程池维护线程的最大线程数
