...
public class RecursiveThreadExecutor {
  ...
  public void waitTermination(Task t) throws InterruptedException {
    if (!executeTaskFlag(t)) waitTaskEnd(t);
  }

  boolean executeTaskFlag(Task t) throws InterruptedException {
    if (Thread.currentThread().isInterrupted()) throw new InterruptedException("Current Thread Interrupted");

    synchronized (t) {
      if (t.interrupted()) throw new InterruptedException("Parent Task Interrupted");
